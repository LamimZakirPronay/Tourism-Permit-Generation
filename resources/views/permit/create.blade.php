@php
    ...
    $oldTeam = old('team', [[]]); 
    // New: Handle old input for multiple vehicles
   ...
   <div class="text-center mb-4 mb-md-5 px-2">
    <div class="main-title-container shadow-lg border border-white border-2 d-flex flex-column flex-md-row align-items-center justify-content-center p-3 p-md-4 gap-3">
        ...
        @if(isset($settings['site_logo']))
            <div class="logo-wrapper border-md-end pe-md-4 mb-3 mb-md-0" style="border-color: #e2e8f0 !important;">
        ...
        <div class="text-center text-md-start">
            <h1 class="text-success fw-bold mb-0" style="font-size: clamp(1.5rem, 5vw, 2.2rem); letter-spacing: -0.5px; line-height: 1.2;">
            ...
            <div class="d-flex align-items-center justify-content-center justify-content-md-start mt-2">
                <span class="badge bg-success-subtle text-success border border-success-subtle px-3 py-2 rounded-pill fw-bold text-uppercase" 
                      style="font-size: 0.65rem; letter-spacing: 1px; white-space: normal; line-height: 1.4;">
                   Managed by Bangladesh Army (Ruma Zone) <br>
        <span style="font-size: 0.75rem; font-weight: 500; text-transform: none;">
            বাংলাদেশ সেনাবাহিনী (রুমা জোন) দ্বারা পরিচালিত
        </span>
                </span>
            </div>
        </div>
...
<style>
    /* Custom CSS to handle the border responsiveness */
                    ...
                    <div class="card-body p-0">
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="area_ids[]" id="area_ids" class="form-control select2-multiple" multiple="multiple" required data-placeholder="Select destination(s)">
    ...
    <div class="text-danger mt-1" style="font-size: 0.75rem; font-weight: 500;">
        ...
        Note: You cannot depart after 6:00 PM. / বিশেষ দ্রষ্টব্য: সন্ধ্যা ৬:০০ টার পর প্রস্থান করা যাবে না।
    </div> </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="d-flex flex-column mt-1">
        <span class="text-muted" style="font-size: 0.75rem;">
            ...
            Please upload the Team Lead's NID card or Passport copy. / অনুগ্রহ করে শুধুমাত্র টিম লিডারের এনআইডি বা পাসপোর্টের কপি আপলোড করুন।
        </span>
        <span class="text-primary mt-1" style="font-size: 0.7rem; font-weight: 500;">
            * If you have multiple pages, please combine them into one PDF. / একাধিক পাতা থাকলে একটি পিডিএফ ফাইলে যুক্ত করে আপলোড করুন।
        </span>
    </div>
</div>
                    </div>
   ...
   {{-- STEP 4: TEAM MEMBERS --}}
    ...
    Team Members (Including Team Lead) / দলের সদস্যগণ (টিম লিডারসহ)
</div>
                        <button type="button" id="add-member-btn" class="btn btn-success btn-sm rounded-pill px-3"><i class="bi bi-person-plus"></i> Add Member / সদস্য যোগ করুন</button>
                    </div>
                    ...
                    <div id="team-members-wrapper">
                        ...
                        <div class="member-card member-row">
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <label class="form-label">Full Name / পূর্ণ নাম<span class="text-required">*</span></label>
                                    <input type="text" name="team[{{ $index }}][name]" value="{{ $member['name'] ?? '' }}" class="form-control" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Father's Name / পিতার নাম<span class="text-required">*</span></label>
                                    <input type="text" name="team[{{ $index }}][fathers_name]" value="{{ $member['fathers_name'] ?? '' }}" class="form-control" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Age / বয়স<span class="text-required">*</span></label>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select name="team[{{ $index }}][gender]" class="form-select" required>
                                        ...
                                        <option value="Other" {{ ($member['gender'] ?? '') == 'Other' ? 'selected' : '' }}>Other / অন্যান্য</option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Category / শ্রেণী<span class="text-required">*</span></label>
                                ...
                                <div class="col-md-5">
                                    <label class="form-label">NID / Passport / এনআইডি<span class="text-required">*</span></label>
                                    <input type="text" name="team[{{ $index }}][id]" value="{{ $member['id'] ?? '' }}" class="form-control" required>
                                    <span class="help-text">For Minors, use parent NID. / নাবালকদের ক্ষেত্রে অভিভাবকের এনআইডি।</span>
                                </div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">Contact / মোবাইল<span class="text-required">*</span></label>
                                    <input type="text" name="team[{{ $index }}][contact_number]" value="{{ $member['contact_number'] ?? '' }}" class="form-control" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Emergency / জরুরি যোগাযোগ<span class="text-required">*</span></label>
                                ...
                                <div class="col-8">
                                    <label class="form-label">Address / স্থায়ী ঠিকানা<span class="text-required">*</span></label>
                ...
                {{-- STEP 3: DRIVER & VEHICLES (MULTIPLE) --}}
                <div class="card form-card">
                    <div class="section-title justify-content-between border-bottom pb-2 mb-4">
                        <div><span class="step-badge">3</span> Driver & Vehicles / চালক ও যানবাহন</div>
                        <button type="button" id="add-vehicle-btn" class="btn btn-outline-success btn-sm rounded-pill px-3"><i class="bi bi-plus-circle"></i> Add Vehicle / যান যোগ করুন</button>
                    </div>
    ...
    <option value="Local Car" {{ ($vehicle['ownership'] ?? '') == 'Local Car' ? 'selected' : '' }}>
        Local (Chander Gari) / চাঁদের গাড়ি
    </option>
    ...
    <option value="Motorbike" {{ ($vehicle['ownership'] ?? '') == 'Motorbike' ? 'selected' : '' }}>
        Motorbike / মোটরসাইকেল
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">Reg No. / রেজি নম্বর<span class="text-required">*</span></label>
                                    <input type="text" name="vehicles[{{ $vIndex }}][reg_no]" class="form-control" value="{{ $vehicle['reg_no'] ?? '' }}" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Driver Name / চালকের নাম<span class="text-required">*</span></label>
                                    <input type="text" name="vehicles[{{ $vIndex }}][driver_name]" class="form-control" value="{{ $vehicle['driver_name'] ?? '' }}" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Driver Contact / মোবাইল<span class="text-required">*</span></label>
                                    <input type="text" name="vehicles[{{ $vIndex }}][driver_contact]" class="form-control" value="{{ $vehicle['driver_contact'] ?? '' }}" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">License / লাইসেন্স</label>
                                    <input type="text" name="vehicles[{{ $vIndex }}][driver_license_no]" class="form-control" value="{{ $vehicle['driver_license_no'] ?? '' }}">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Driver NID / চালকের এনআইডি</label>
                                    <input type="text" name="vehicles[{{ $vIndex }}][driver_nid]" class="form-control" value="{{ $vehicle['driver_nid'] ?? '' }}">
                                </div>
                            </div>
                        </div>
                        @endforeach
                ...
                <div class="card form-card border-top border-success border-4">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <select class="form-select select2" name="tour_guide_id" required style="width: 100%;">
                <option value="">-- Search & Choose Guide / গাইড খুঁজুন ও নির্বাচন করুন --</option>
                ...
                <div class="sticky-top" style="top: 20px;">
                    <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
                        <div class="bg-success p-4 text-white text-center">
                            <h5 class="mb-0 fw-bold">Payment Summary / পেমেন্ট বিবরণ</h5>
                        </div>
                        <div class="card-body p-4 bg-white">
                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">Unit Fee (Adult) / জনপ্রতি ফি</span>
                                <span class="fw-bold">৳{{ number_format($adultFee, 2) }}</span>
                            </div>
                            <hr>
                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">Total Members / মোট সদস্য</span>
                                <span id="total_members_count" class="fw-bold">0</span>
                            </div>
                            <div class="d-flex justify-content-between mb-3">
                                <span class="text-muted">Chargeable / ফি প্রযোজ্য</span>
                                <span id="adult_members_count" class="fw-bold text-success">0</span>
                            </div>
                            <div class="bg-light p-3 rounded-3 mt-4 border">
                                <div class="d-flex justify-content-between align-items-center">
                                    <span class="fw-bold text-dark">Total Payable / মোট প্রদেয়</span>
...
<script>
    ...
    // Initialize counts based on existing old input or default to 1
    let memberCount = {{ max(count($oldTeam), 1) }};
    let vehicleCount = {{ max(count($oldVehicles), 1) }};
    ...
    // Helper function to re-index names after removal to prevent array gaps
...
$(document).ready(function() {
    $('.select2').select2({
        theme: 'bootstrap-5', // Makes it look like a Bootstrap 5 input
        placeholder: '-- Choose Guide / গাইড নির্বাচন করুন --',
        allowClear: true
    });
});
</script>
